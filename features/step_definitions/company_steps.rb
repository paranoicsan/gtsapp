...
When /^Я нахожусь на странице компании на повторном рассмотрении$/ do
  step %Q{Существует 1 компаний на повторном рассмотрении}
  visit company_path @company
end
When /^Я нахожусь на странице компании, для филиала которой есть (\d+) телефонов$/ do |cnt|
  step %Q{Существует филиал с #{cnt} телефонами}
