Feature: У филиалов может быть только один адрес
  ...
  Scenario: Пользователь видит существующий адрес на странице филиала
    ...
    And Я перехожу на страницу филиала
  ...
  @javascript
  Scenario: Агент может выбрать населённый пункт с автозаполнением
    Given Я - "агент", авторизованный в системе
    When Я нахожусь на странице изменения адреса
    Then Я могу выбрать населённый пункт автозаполнением

  @javascript
  Scenario: Агент может выбрать улицу с автозаполнением
    Given Я - "агент", авторизованный в системе
    When Я нахожусь на странице изменения адреса
  ...
  Scenario: Значение введённой улицы сбрасывается при смене населённого пункта
    Given Я - "агент", авторизованный в системе
    And Я нахожусь на странице изменения адреса
    When Я удаляю введённый населенный пункт
    Then Значение улицы сбрасывается

  @javascript
  Scenario: Пользователь не может ввести улицу, если не выбран неселённый пункт
    Given Я - "агент", авторизованный в системе
    When Я нахожусь на странице изменения адреса
  ...
  @javascript
  Scenario: Пользователь не может сохранить адрес без указания населённого пункта
    Given Я - "агент", авторизованный в системе
    When Я нахожусь на странице создания адреса
    Then Я не могу сохранить адрес не указав населённый пункт

  @javascript @focus
  Scenario: Пользователь не может сохранить адрес без указания улицы
    Given Я - "агент", авторизованный в системе
    When Я нахожусь на странице создания адреса
    And Я могу выбрать населённый пункт автозаполнением
    Then Я не могу сохранить адрес не указав улицу
