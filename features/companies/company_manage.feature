Feature: Создание, изменение и удаление компании
  ...
  Scenario: Пользователь не может добавить компанию без имени
    ...
    When Я нахожусь на странице создания компании
    Then Поле для названия подсвечиватеся красным
    And Я не могу сохранить компанию

  @javascript
  Scenario: Пользователь не может добавить компанию c названием, которое уже занято
    Given Я - "агент", авторизованный в системе
    And Существует 1 компаний
    When Я нахожусь на странице создания компании
    And Я ввожу название, которое уже занято
    Then Поле для названия подсвечиватеся красным
    And Я не могу сохранить компанию
    And Я вижу сообщение, что имя занято

  @javascript
  Scenario: Пользователь не может создать компанию без указания рубрикатора
    Given Я - "агент", авторизованный в системе
    When Я нахожусь на странице создания компании
    And Я ввожу "test" в поле "company_title"
    Then Я  могу сохранить компанию
    And Я не могу сохранить компанию, не выбрав рубрикатор

  @javascript @wip
  Scenario: Пользователь не может изменить компанию без указания рубрикатора
    Given Я - "агент", авторизованный в системе
    And Существует 1 компаний
    When Я нахожусь на странице изменения компании
    Then Я не могу сохранить компанию, не выбрав рубрикатор
