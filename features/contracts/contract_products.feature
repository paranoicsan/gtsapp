Feature: Договора могут содержать в себе неограниченное количество продуктов
  ...
  Scenario: Администратор может добавлять продукты к НЕ активному договору на странице договора
    ...
    And Я не вижу таблицу "products"
    When Я выбираю продукт "Реклама на второй странице"
    And Я нажимаю на ссылку "Добавить" с ключом "add_prod_link"
    Then Я вижу "Реклама на второй странице" в ряду 2 таблице "products"
  ...
  @wip
  Scenario: Агент не может добавлять продукты к активному договору по прямой ссылке
    Given Я - пользователь "t_agent" с паролем "1111"
